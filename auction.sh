...
display_auction_info () {
    ...
    local credit_request=$(jq -r '.auction_info.credit_request' <<<$auctioninfo)
    convert_denom $credit_request $saledecimals
    echo -e "${BLUE}Credit Request: ${GRN}$denom"
...
then
    ...
    credit_request=$(jq -r '.[].credit_request' <<<"$auctioninfo")
...
EOF
            ...
            then
                bidcmd="d"
                goodinp=true
            elif [[ "$lowcase" == "calculate" ]] || [[ "$lowcase" == "c" ]]
            then
                bidcmd="c"
                goodinp=true
            elif [[ "$lowcase" == "quit" ]] || [[ "$lowcase" == "q" ]]
...
# calculate proposal
        ...
        then
            ...
            while [ $goodinp == false ]
            do
              echo -e "\nEnter your proposal to calculate"
              get_json
            done
            proposal=$amount
            resp=$(secretcli q compute query $auctionaddr "{\"calculate_proposal\": {\"proposal\":$proposal}}" --trust-node=true -o json)
            echo $resp
# view active bid
        elif [[ $bidcmd == 'v' ]]
        then
...
\"$bidaddr\"},\"credit_request\":$credit_request,\"description\":\"$descinp\"}" --from $addr \
